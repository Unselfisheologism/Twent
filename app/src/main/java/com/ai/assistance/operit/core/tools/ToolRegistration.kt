...
fun registerAllTools(handler: AIToolHandler, context: Context) {
    ...
    // OpenUI Generative UI tool - renders OpenUI Lang code as interactive UI
    handler.registerTool(
            name = "render_openui",
            dangerCheck = { false },
            descriptionGenerator = { tool ->
                val title = tool.parameters.find { it.name == "title" }?.value ?: "OpenUI"
                s(R.string.toolreg_render_openui_desc, title)
            },
            executor = { tool ->
                ...
                if (code.isBlank()) {
                    ToolResult(
                            ...
                            success = false,
                            result = StringResultData(""),
                            error = "OpenUI Lang code is required for render_openui"
                    )
                } else {
                    try {
                        // Load the OpenUI HTML template from assets
                        val template = context.assets.open("openui/template.html")
                            .bufferedReader()
                            .use { it.readText() }

                        // Escape the code for safe embedding in JavaScript string
                        ...
                        val scaffold = com.ai.assistance.operit.data.miniapp.MiniAppScaffold.FromFiles(
                                ...
                                name = title,
                                type = miniAppType,
                                description = "OpenUI: $title",
                                entryFile = "index.html",
                                metadata = mapOf(
                                    "created_by" to "openui_tool",
                                    "openui_code" to code,
                        ...
                        val manager = com.ai.assistance.operit.data.miniapp.MiniAppManager.getInstance(context)
                        val ensureName = runBlocking { manager.ensureUniqueName(title, miniAppType) }
                        val finalScaffold = scaffold.copy(name = ensureName)
                        val result = runBlocking { manager.createMiniApp(finalScaffold) }
                        ...
                        result.fold(
                                ...
                                onFailure = { e ->
                                    ToolResult(
                                            ...
                                            success = false,
                                            result = StringResultData(""),
                                            error = "Failed to create OpenUI render: ${e.message}"
                                    )
                                }
                        )
                    } catch (e: Exception) {
                        ToolResult(
                                ...
                                success = false,
                                result = StringResultData(""),
                                error = "Error rendering OpenUI: ${e.message}"
                        )
                    }
                }
            }
