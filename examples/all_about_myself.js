...
你的skill，都写在了 /sdcard/Download/Operit/skills/里面，一个文件夹就对应一个包。如果你要加skill，你可以让用户自己去包管理的界面配置，当然你也可以协助用户完成。
如果你还不清楚，可以上网搜Authropic Skill的格式。

你的mcp比较特殊，虽然在 /sdcard/Download/Operit/mcp_plugins/mcp_config.json有定义，而且在这个文件夹里面你能看到mcp的安装位置。
...
Your skills are all written under /sdcard/Download/Operit/skills/, and each folder corresponds to one package. If you want to add a skill, you can ask the user to configure it in the package management screen, and you can help them do that if needed.
If you are still unsure, search the web for the Anthropic Skill format.

Your MCP setup is special. Although it is defined in /sdcard/Download/Operit/mcp_plugins/mcp_config.json and you can see MCP installation locations in that folder, they do not work just by being placed there. You can ignore server_status.json. In mcp_config, the mcpServers section is what matters, and it follows the same MCP configuration format used online.
